...
typename types<Frame>::ThetaPhi ThetaPhi<Frame>::function(
...
template <typename Frame>
typename types<Frame>::OneForm Rhat<Frame>::function(
...
template <typename Frame>
typename types<Frame>::Jacobian Jacobian<Frame>::function(
    const typename types<Frame>::ThetaPhi& theta_phi) noexcept {
  const auto& theta = theta_phi.get(0);
  const auto& phi = theta_phi.get(1);
...
template <typename Frame>
typename types<Frame>::InvJacobian InvJacobian<Frame>::function(
    const typename types<Frame>::ThetaPhi& theta_phi) noexcept {
  const auto& theta = theta_phi.get(0);
  const auto& phi = theta_phi.get(1);
  const DataVector sin_phi = sin(phi);
...
template <typename Frame>
typename types<Frame>::InvHessian InvHessian<Frame>::function(
    const typename types<Frame>::ThetaPhi& theta_phi) noexcept {
  const auto& theta = theta_phi.get(0);
  const auto& phi = theta_phi.get(1);
  const DataVector sin_phi = sin(phi);
  ...
  // R^2 d^2 th/(dxdy)
  inv_hess.get(0, 0, 1) = -cot_theta * sin_phi_cos_phi * f1;
  // R^2 d^2 th/(dxdz)
  inv_hess.get(0, 0, 2) = (sin_sq_theta - cos_sq_theta) * cos_phi;
  // R^2 d^2 th/(dydx)
  inv_hess.get(0, 1, 0) = -cot_theta * sin_phi_cos_phi * f1;
  // R^2 d^2 th/(dy^2)
  ...
  inv_hess.get(1, 0, 0) = sin_phi_cos_phi * (1 + sin_sq_theta) * csc_theta;
  ...
  inv_hess.get(1, 1, 1) = -sin_phi_cos_phi * (1 + sin_sq_theta) * csc_theta;
...
template <typename Frame>
typename types<Frame>::Vector CartesianCoords<Frame>::function(
...
template <typename Frame>
typename types<Frame>::OneForm DxRadius<Frame>::function(
    const ::Strahlkorper<Frame>& strahlkorper, const DataVector& radius,
...
template <typename Frame>
typename types<Frame>::SecondDeriv D2xRadius<Frame>::function(
    const ::Strahlkorper<Frame>& strahlkorper, const DataVector& radius,
...
template <typename Frame>
DataVector NablaSquaredRadius<Frame>::function(
    const ::Strahlkorper<Frame>& strahlkorper, const DataVector& radius,
    const typename types<Frame>::ThetaPhi& theta_phi) noexcept {
  const auto derivs =
      strahlkorper.ylm_spherepack().first_and_second_derivative(radius);
...
template <typename Frame>
typename types<Frame>::OneForm NormalOneForm<Frame>::function(
...
template <typename Frame>
std::array<typename types<Frame>::Vector, 2> Tangents<Frame>::function(
    const ::Strahlkorper<Frame>& strahlkorper, const DataVector& radius,
