...
class Arthropod : public Animal {
 public:
  Arthropod() = default;
  explicit Arthropod(size_t num_legs) : number_of_legs_(num_legs){};
  explicit Arthropod(CkMigrateMessage* /*m*/) {}
  size_t number_of_legs() const noexcept final { return number_of_legs_; }
  void set_number_of_legs(size_t num_legs) noexcept final {
...
void TestArrayChare<Metavariables>::run_test_one() noexcept {
  ...
  Parallel::mutate<weight, modify_value<double>>(global_cache_proxy_,
                                                 std::make_tuple<double>(150));
  run_test_two();
}
...
void TestArrayChare<Metavariables>::run_test_two() noexcept {
  ...
  if (Parallel::mutable_cache_item_is_ready<weight>(
          *global_cache_proxy_.ckLocalBranch(),
          [&callback](const double& weight_l) -> boost::optional<CkCallback> {
            return weight_l == 150 ? boost::none
                                   : boost::optional<CkCallback>(callback);
          })) {
    auto& local_cache = *global_cache_proxy_.ckLocalBranch();
    ...
    Parallel::mutate<email, modify_value<std::string>>(
        global_cache_proxy_,
        std::make_tuple<std::string>("albert@einstein.com"));
...
void TestArrayChare<Metavariables>::run_test_three() noexcept {
  ...
  if (Parallel::mutable_cache_item_is_ready<email>(
          *global_cache_proxy_.ckLocalBranch(),
          [&callback](
              const std::string& email_l) -> boost::optional<CkCallback> {
            return email_l == "albert@einstein.com"
                       ? boost::none
                       : boost::optional<CkCallback>(callback);
          })) {
    auto& local_cache = *global_cache_proxy_.ckLocalBranch();
    SPECTRE_PARALLEL_REQUIRE("albert@einstein.com" ==
                             Parallel::get<email>(local_cache));
...
void TestArrayChare<Metavariables>::run_test_four() noexcept {
  ...
  if (Parallel::mutable_cache_item_is_ready<animal>(
          *global_cache_proxy_.ckLocalBranch(),
          [&callback](const Animal& animal_l) -> boost::optional<CkCallback> {
    ...
    Parallel::mutate<animal_base, modify_number_of_legs>(
        global_cache_proxy_, std::make_tuple<size_t>(30));
...
void Test_GlobalCache<Metavariables>::run_single_core_test() noexcept {
  ...
  Parallel::mutate<weight, modify_value<double>>(cache,
                                                 std::make_tuple<double>(150));
  Parallel::mutate<email, modify_value<std::string>>(
      cache, std::make_tuple<std::string>("nobody@nowhere.com"));
  ...
  Parallel::mutate<email, modify_value<std::string>>(
      cache, std::make_tuple<std::string>("isaac@newton.com"));
  SPECTRE_PARALLEL_REQUIRE("isaac@newton.com" == Parallel::get<email>(cache));
  // Make the arthropod into a spider.
  Parallel::mutate<animal, modify_number_of_legs>(cache,
                                                  std::make_tuple<size_t>(8));
  ...
  // Make the arthropod into a Scutigera coleoptrata.
  Parallel::mutate<animal_base, modify_number_of_legs>(
      cache, std::make_tuple<size_t>(30));
