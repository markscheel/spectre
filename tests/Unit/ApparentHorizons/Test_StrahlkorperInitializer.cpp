...
SPECTRE_TEST_CASE("Unit.ApparentHorizons.StrahlkorperInitializer",
  ...
  const StrahlkorperInitializers::Sphere sphere{l_max, radius, center};

  const auto strahlkorper = sphere.create_strahlkorper();
  const Strahlkorper test_strahlkorper(l_max, l_max, radius, center);
...
SPECTRE_TEST_CASE("Unit.ApparentHorizons.StrahlkorperInitializer.Factory",
                  "[ApparentHorizons][Unit]") {
  const auto creator = test_factory_creation<StrahlkorperInitializer>(
      "  Sphere:\n"
      "    Lmax: 5\n"
      "    Radius: 2\n"
      "    Center: [3,4,5]");
  ...
  const Strahlkorper test_strahlkorper(5, 5, 2.0, {{3.0, 4.0, 5.0}});
  CHECK(strahlkorper == test_strahlkorper);
}
