...
void test_inverse(const size_t ell_max) {
  ...
  Variables<tmpl::list<MyTag>> A(spectral_size, 0.0), B(spectral_size, 0.0),
      C(spectral_size, 0.0);
  ...
  for (auto& a : get<MyTag>(A)) {
    for (it.reset(); it; ++it) {
      ...
      if (it.l() < ell_max + 1 - get<MyTag>(A).rank() and
  ...
  // Convert A to B.
  std::span<double> a_span(A.data(), A.size());
  std::span<double> b_span(B.data(), B.size());
  ...
  // Convert B to C
  std::span<double> c_span(C.data(), C.size());
  ...
  sphere_to_cart.increment_multiply_on_right(make_not_null(&c_span), 0, b_span,
                                             0);
  // C should equal A.
  const auto& a_vector = get<MyTag>(A);
  const auto& c_vector = get<MyTag>(C);
