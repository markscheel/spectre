...
// Here we initialize the type with its default value.
namespace OptionTags {
struct VectorOfDoubles {
  static std::string name() noexcept { return "VectorOfDoubles"; }
  static constexpr OptionString help = "Options for vector of doubles";
  using type = std::vector<double>;
  static type default_value() noexcept { return type{}; }
};
}  // namespace OptionTags
...
struct add_new_stored_double {
  ...
  static void apply(db::DataBox<tmpl::list<DbTags...>>& /*box*/,
    ...
    Parallel::mutate<Tags::VectorOfDoubles,
                     MutationFunctions::add_stored_double>(
        cache.thisProxy, std::make_tuple<double>(42.0));
    /// [mutate_global_cache_item]
  }
};
  ...
  static bool is_ready(const db::DataBox<DbTags>& /*box*/,
    ...
    return ::Parallel::mutable_cache_item_is_ready<Tags::VectorOfDoubles>(
        ...
        [&callback](const std::vector<double>& VectorOfDoubles)
            -> boost::optional<CkCallback> {
          return VectorOfDoubles.empty() ? boost::optional<CkCallback>(callback)
                                         : boost::none;
        });
  }
  /// [check_mutable_cache_item_is_ready]
...
//
// 2) UseMutatedCacheCompnent has a single iterable_action whose
