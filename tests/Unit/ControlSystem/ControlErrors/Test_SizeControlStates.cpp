...
void test_size_control_update_work(
    ...
    const control_system::size::Info& expected_info,
    const control_system::size::State& size_control_state,
...
void test_size_control_update() {
  ...
  auto initial_state = control_system::size::Label::Initial;
  ...
  auto do_test = [&initial_state, &damping_time, &min_comoving_char_speed,
                  ...
                  &original_target_char_speed](
                     const control_system::size::Label final_state,
    ...
    auto size_control_state =
        [&initial_state]() -> std::unique_ptr<control_system::size::State> {
      switch (initial_state) {
        case control_system::size::Label::AhSpeed:
          return std::make_unique<control_system::size::States::AhSpeed>();
        case control_system::size::Label::DeltaR:
          return std::make_unique<control_system::size::States::DeltaR>();
        default:
          return std::make_unique<control_system::size::States::Initial>();
      }
    }();
  ...
  // should do nothing
  do_test(control_system::size::Label::Initial, false, 0.0,
          original_target_char_speed);

  // should go into DeltaR state
  min_comoving_char_speed = 0.02;
  do_test(control_system::size::Label::DeltaR, true, 0.0,
          original_target_char_speed);

  // Make deltar cross zero after damping time.
  crossing_time_info =
      control_system::size::CrossingTimeInfo(0.0, 0.0, 1.1 * damping_time);
  do_test(control_system::size::Label::DeltaR, true, 0.0,
          original_target_char_speed);

  // Make deltar cross zero before damping time.
  crossing_time_info =
      control_system::size::CrossingTimeInfo(0.0, 0.0, 0.9 * damping_time);
  do_test(control_system::size::Label::DeltaR, true, 0.9 * damping_time,
          original_target_char_speed);

  // Make deltar cross zero before damping time, faster than char speed.
  crossing_time_info = control_system::size::CrossingTimeInfo(
      0.91 * damping_time, 0.0, 0.9 * damping_time);
  do_test(control_system::size::Label::DeltaR, true, 0.9 * damping_time,
          original_target_char_speed);

  // Make deltar cross zero before damping time, same as char speed.
  crossing_time_info = control_system::size::CrossingTimeInfo(
      0.9 * damping_time, 0.0, 0.9 * damping_time);
  do_test(control_system::size::Label::DeltaR, true, 0.9 * damping_time,
          original_target_char_speed);

  // Make deltar cross zero before damping time, slower than char speed.
  crossing_time_info = control_system::size::CrossingTimeInfo(
      0.89 * damping_time, 0.0, 0.9 * damping_time);
  do_test(control_system::size::Label::AhSpeed, true, 0.89 * damping_time,
          min_char_speed * 1.01);

  // Now do DeltaR tests
  initial_state = control_system::size::Label::DeltaR;
  min_comoving_char_speed = -0.02;
  crossing_time_info = control_system::size::CrossingTimeInfo(0.0, 0.0, 0.0);

  // Should do nothing
  do_test(control_system::size::Label::DeltaR, false, 0.0,
          original_target_char_speed);

  // Should change suggested time scale
  min_comoving_char_speed = 0.02;
  do_test(control_system::size::Label::DeltaR, false, 0.99 * damping_time,
          original_target_char_speed);
  ...
  crossing_time_info =
      control_system::size::CrossingTimeInfo(0.0, 0.0, 0.999 * damping_time);
  do_test(control_system::size::Label::DeltaR, false, 0.0,
          original_target_char_speed);
  ...
  crossing_time_info =
      control_system::size::CrossingTimeInfo(0.0, 0.0, 0.9 * damping_time);
  do_test(control_system::size::Label::DeltaR, false, 0.9 * damping_time,
          original_target_char_speed);
  ...
  crossing_time_info = control_system::size::CrossingTimeInfo(
      0.89 * damping_time, 0.0, 0.9 * damping_time);
  do_test(control_system::size::Label::DeltaR, false, 0.89 * damping_time,
          original_target_char_speed);
  ...
  crossing_time_info = control_system::size::CrossingTimeInfo(
      0.89 * damping_time, 1.e12, 0.9 * damping_time);
  min_comoving_char_speed = 0.02;
  do_test(control_system::size::Label::AhSpeed, true, 0.89 * damping_time,
          min_char_speed * 1.01);

  // Now do AhSpeed tests
  initial_state = control_system::size::Label::AhSpeed;
  min_comoving_char_speed = -0.02;
  control_err_delta_r = 0.03;
  crossing_time_info = control_system::size::CrossingTimeInfo(0.0, 0.0, 0.0);

  // Should do nothing.
  do_test(control_system::size::Label::AhSpeed, false, 0.0,
          original_target_char_speed);
  ...
  min_char_speed = 0.01;
  min_comoving_char_speed = 0.02;
  crossing_time_info = control_system::size::CrossingTimeInfo(0.0, 1.e-10, 0.0);
  do_test(control_system::size::Label::AhSpeed, false, 0.0,
          original_target_char_speed);

  // Now it should go to DeltaR because comoving crossing time is large.
  crossing_time_info = control_system::size::CrossingTimeInfo(0.0, 100.0, 0.0);
  do_test(control_system::size::Label::DeltaR, true, 0.0,
          original_target_char_speed);
  ...
  crossing_time_info =
      control_system::size::CrossingTimeInfo(0.0, 0.0, 19.0 * damping_time);
  do_test(control_system::size::Label::DeltaR, true, 19.0 * damping_time,
          original_target_char_speed);

  // But now with comoving_char_speed negative it should stay in AhSpeed
  // state, but with a change in target speed.
  min_comoving_char_speed = -0.02;
  do_test(control_system::size::Label::AhSpeed, true, damping_time,
  ...
  min_comoving_char_speed = 0.02;
  crossing_time_info =
      control_system::size::CrossingTimeInfo(1.e10, 0.0, 19.0 * damping_time);
  do_test(control_system::size::Label::AhSpeed, true, damping_time,
          0.125 * min_char_speed);

  // .. but not if the delta_r crossing time is small enough.
  crossing_time_info =
      control_system::size::CrossingTimeInfo(1.e10, 0.0, 4.99 * damping_time);
  do_test(control_system::size::Label::DeltaR, true, 4.99 * damping_time,
          original_target_char_speed);
  ...
  crossing_time_info = control_system::size::CrossingTimeInfo(
      0.89 * damping_time, 0.0, 0.9 * damping_time);
  do_test(control_system::size::Label::AhSpeed, false, 0.89 * damping_time,
          original_target_char_speed);
  ...
  crossing_time_info = control_system::size::CrossingTimeInfo(
      0.98999 * damping_time, 0.0, 0.99 * damping_time);
  do_test(control_system::size::Label::AhSpeed, false, 0.98999 * damping_time,
          min_char_speed * 1.01);
  ...
  min_char_speed = original_target_char_speed * 1.09999;
  crossing_time_info = control_system::size::CrossingTimeInfo(
      0.99001 * damping_time, 0.0, 0.992 * damping_time);
  do_test(control_system::size::Label::DeltaR, true, 0.0,
          original_target_char_speed);
}
...
void test_size_control_signal() {
  ...
  const control_system::size::Info info{
      control_system::size::Label::Initial, 1.1, 0.011, 1.e-3, 2.e-3, false};
