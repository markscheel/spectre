...
void test_parse_errors() {
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
          ...
          Options::Context{false, {}, 1, 1}),
      Catch::Matchers::Contains("Outer boundary radius is too small"));
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
  ...
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
          ...
          create_inner_boundary_condition(), create_outer_boundary_condition(),
          Options::Context{false, {}, 1, 1}),
      Catch::Matchers::Contains("Radii are expected to be positive"));
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
          {{2.0, 0.05, 0.0}}, {-5.0, 0.05, 0.0}, 1.0, -0.4, 25.0, 1, 3, nullptr,
          create_inner_boundary_condition(), create_outer_boundary_condition(),
  ...
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
          ...
          Options::Context{false, {}, 1, 1}),
      Catch::Matchers::Contains(
          "radius_A should not be smaller than radius_B"));
  CHECK_THROWS_WITH(
      domain::creators::CylindricalBinaryCompactObject(
...
// [[Timeout, 15]]
SPECTRE_TEST_CASE(
    "Unit.Domain.Creators.CylindricalBinaryCompactObject.FactoryTests",
    "[Domain][Unit]") {
  test_connectivity();
